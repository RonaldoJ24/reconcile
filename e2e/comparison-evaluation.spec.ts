...
test.describe('comparison and evaluation', () => {
  ...
  test('real backend persists comparison observations and serves the packaged evaluation', async ({ page }) => {
    ...
    // Rules, ranker, DeepSeek and GPT-6 Luna for the held-out split and for all cases.
    await expect(page.locator('.evaluation-v2-results tbody tr')).toHaveCount(8)
    await expect(page.locator('.evaluation-v2-results')).toContainText('21 of 34')
    await expect(page.locator('.evaluation-v2-results')).toContainText('15 of 34')
